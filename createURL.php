...
function createURLHouse(){
    ...
    */
    ...
    if(strcasecmp($response['floorCountMin'], "") != 0) {
        $request['search%5Bfilter_float_total_floors%3Afrom%5D'] = $response['floorCountMin'];
    }

    // дописать мин этажность
    if(strcasecmp($response['floorCountMax'], "") != 0) {
        $request['search%5Bfilter_float_floor%3Ato%5D'] = $response['floorCountMax'];
    }

    // дописать мин количество комнат
    if(strcasecmp($response['roomNumberMin'], "") != 0) {
        $request['search%5Bfilter_float_number_of_rooms%3Afrom%5D'] = $response['roomNumberMin'];
    }

    // дописать макс количество комнат
    if(strcasecmp($response['roomNumberMax'], "") != 0) {
        $request['search%5Bfilter_float_number_of_rooms%3Ato%5D'] = $response['roomNumberMax'];
    }

    // дописать мин общая площадь
    if(strcasecmp($response['areaMin'], "") != 0) {
        $request['search%5Bfilter_float_total_area%3Afrom%5D'] = $response['areaMin'];
    }

    // дописать маск общая площадь
    if(strcasecmp($response['areaMax'], "") != 0) {
        $request['search%5Bfilter_float_total_area%3Ato%5D'] = $response['areaMax'];
    }
...
function createURLFlat(){
    ...
    */
    ...
    // дописать макс этажность
    if(strcasecmp($response['floorCountMax'], "") != 0) {
        $request['search%5Bfilter_float_total_floors%3Ato%5D'] = $response['floorCountMax'];
    }

    // дописать мин этаж
    if(strcasecmp($response['floorMin'], "") != 0) {
        $request['search%5Bfilter_float_floor%3Afrom%5D'] = $response['floorMin'];
    }

    // дописать макс этаж
    if(strcasecmp($response['floorMax'], "") != 0) {
        $request['search%5Bfilter_float_floor%3Ato%5D'] = $response['floorMax'];
    }
